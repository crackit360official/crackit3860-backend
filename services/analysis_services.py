...
class AnalyticsService:
    ...
    # ---------------- PUBLIC API ----------------
    async def get_analytics(self, user_id: str, period: str):
        cache_key = f"analytics:{user_id}:{period}"
        return await self.mongo_cache(
            cache_key,
            lambda: self._compute(user_id, period),
        )
    ...
    # ---------------- CORE COMPUTE ----------------
    async def _compute(self, user_id: str, period: str):
        ...
        consistency = (
            round(statistics.stdev(accuracies), 2)
            if len(accuracies) > 1
            else 0
        )
